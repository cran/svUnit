...
use of unit tests as in extreme programming (test first – code second): 
\begin{itemize}
...
the test is run, a \var{RUnitTestData} object is created that contains
...
\code{owarn <- options(warn = 2)\$warn}, run the code that should
generate a warning inside \code{checkException()}, and then restore
default warning behavior with \code{options(warn = owarn)}.%
...
\code{msg} where you can provide a (short) message to print in front
of each text in the report. These functions return invisibly: \code{TRUE}
if the test succeeds, or \code{FALSE} if it fails (code is executed
correctly, but does not pass the test), and \code{NA} if there was
an error (the \R{} code of the test was not executed correctly).
Moreover, these functions record the results, the context of the test
and the timing in a logger (object \var{svSuiteData} inheriting from
\var{environment}) called \var{.Log} and located in the user’s workspace.
So, executing a series of assertions and getting a report is simply
done as (in its simplest form, you can use the various \code{checkXXX()}
...
in the \var{eval} category: there is no better context known for
...
the graphical tree in the Komodo’s \textbf{R Unit} tab, but the simple
...
as separate \var{svTest} objects) by using \code{makeUnit()}. Here
is how you promote the test associated with our \code{Square()} function
...
moment, let’s discuss a little bit how such test units should be organized
in \R{} package.
...
\bibitem{Konig}König, T., K. Jünemann \& M. Burger, 2007. RUnit –
